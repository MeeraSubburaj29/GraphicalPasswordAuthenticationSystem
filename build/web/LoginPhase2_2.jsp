...
<html lang="en">
	...
	document.getElementById("image2").style.left = (pos_x-1) ;
	document.getElementById("image2").style.top = (pos_y-15) ;
	document.getElementById("image2").style.visibility = "visible" ;
	//document.getElementById("x1").value = pos_x;
	//document.getElementById("y1").value = pos_y;
                document.form1.x2.value = pos_x;
	document.form1.y2.value = pos_y;
            }
                                ...
                                <main class="form-signin w-150 m-auto">
                                    ...
                                    <%
                                        ...
                                        if(rs.next()){
                                            ...
                                            session.setAttribute("Image2", imageName);
                                        }
                                        rs.close();
                                        ps.close();
                                    %>
                                    <form name="form1" method="post" action="LoginPhase2_2.jsp">

                                        <h1 class="h3 mb-3 fw-normal" style="font-size: 25px;font-weight: bold;">
                                            Phase2 Login
                                        </h1>
                                        <h2>
                                            (Image Click Point-Image2)
                                        </h2>
                                        <h5>
                                           One Click Point on Image
                                        </h5>                                        
                                        <img id="image2" name="image2" src="${sessionScope.UserId}/<%=imageName%>" onclick="getImagePosition(event);" />
                                        <input type="hidden" name="x2" style="width: 65px;"/>
                                        <input type="hidden" name="y2" style="width: 65px;"/>
                                        ...
                                        <%
                                            if(request.getParameter("s1")!=null){
                                                x = Integer.parseInt(request.getParameter("x2"));
                                                y = Integer.parseInt(request.getParameter("y2"));
                                                session.setAttribute("Image2_X", x);
                                                session.setAttribute("Image2_Y", y);
            
                                                response.sendRedirect("LoginPhase2_3.jsp");
                                            }
                                        %>
                                </main>
                            </div>
